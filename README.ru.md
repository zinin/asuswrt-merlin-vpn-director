...
Вкладка **Settings** показывает текущую версию, последний релиз на GitHub и его changelog. Кнопка «Update to vX» скачивает релиз и перезапускает и Web UI, и Telegram-бота; страница опрашивает версию и сама перезагружается, когда новая сборка поднялась. Сессия входа переживает обновление.
...
> Переход **на** первый релиз с единым обновлением выполняет прежний обновлятор бота, который не знает про Web UI. После этого перехода один раз запустите `install.sh`; все последующие обновления охватывают оба демона.
