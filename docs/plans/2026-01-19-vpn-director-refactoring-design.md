...
**Q1: Обратная совместимость для внешних пользователей?**

Некоторые пользователи могут вызывать старые скрипты напрямую (`ipset_builder.sh`, `tunnel_director.sh`, `xray_tproxy.sh`).

Варианты:
1. **Без совместимости** — просто удалить старые скрипты, обновить документацию
2. **Thin wrappers** — оставить старые имена как обёртки, вызывающие `vpn-director.sh`
3. **Симлинки** — создать симлинки на vpn-director.sh (не сработает из-за разных аргументов)

**Решение:** Вариант 1 (без совместимости). Это внутренний инструмент, внешних пользователей нет. Обновление CLAUDE.md и документации достаточно.
