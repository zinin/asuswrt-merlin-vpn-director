...
4. `update.sh` is generated from the daemon table and run detached, from `/` rather than from the update directory — the bot deletes that directory the moment it reports success, and a working directory that no longer exists is inherited by every daemon the script starts: monit then refuses to run at all, and every shell those daemons spawn prints `getcwd` errors into the Web UI's status output. For the same reason the script's `log` helper survives the loss of its own file, or `set -e` would end the script mid-way through its last steps
